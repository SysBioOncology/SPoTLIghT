...
def proba_maps(
    ...
    fig, axes = plt.subplots(
        1, len(cell_types), sharex=False, sharey=False, figsize=(20, 5)
    )
    ...
    for j, cell_type in enumerate(cell_types):
        ...
        sns.heatmap(
            temp,
            cmap=cmap_cell_types[cell_type],
            vmin=vmin,
            vmax=vmax,
            ax=axes[j],
            cbar=False,
    ...
    for j, cell_type in enumerate(cell_types):
        axes[j].set_ylabel(
            cell_type.replace("_", " ").replace("purity", "cells"), size=fontsize
        )
        ...
        sm.set_array([])
        axins = inset_axes(
            axes[j], width="100%", height="3%", loc="lower center", borderpad=-2
        )
        fig.colorbar(
            sm,
            cax=axins,
    ...
    plt.savefig(
        f"{output_dir}/{slide_submitter_id}_cell_type_maps.pdf",
        bbox_inches="tight",
        dpi=600,
    )
...
def create_boxplots(
    ...
    labelpad = 10
    fontsize = 12
    subtype_palette = dict(
        zip(["F", "IE/F", "IE", "D"], sns.color_palette("pastel")[:4])
    )
    ...
    strip_params.update({"palette": "dark:black", "size": 1, "dodge": True})
    ...
    if compare_subtypes:
        ...
        for x in sorted(feature_data[level].unique()):
            ...
            subtype_combinations = [
                list(comparison) for comparison in list(itertools.combinations(temp, 2))
            ]
            ...
            for (pair1, subtype1), (pair2, subtype2) in subtype_combinations:
                H, p = mannwhitneyu(
                    feature_data.loc[
                        (feature_data[level] == pair1) & (feature_data.MFP == subtype1),
                        feature,
                    ],
                    feature_data.loc[
                        (feature_data[level] == pair2) & (feature_data.MFP == subtype2),
                        feature,
                    ],
                )
                H_values.append(H)
        ...
        if len(sign_pairs) > 0:
            ...
            annot.new_plot(ax=g, pairs=sign_pairs, **box_plot_params)
            (
                annot.configure(test=None, test_short_name="mannwhitneyu", verbose=0)
    ...
    if output_dir is not None:
        if plot_name is None:
            ...
            plt.savefig(
                f"{output_dir}/boxplots_{plot_name}.pdf", bbox_inches="tight", dpi=600
            )
