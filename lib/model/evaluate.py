...
def compute_tile_predictions(
    ...
    if (
        ...
        full_model_path = Path(
            models_dir, f"{[s for s in subfolders if cell_type in s][0]}"
        )

    # Load models and scalers
    outer_models = joblib.load(Path(full_model_path, "outer_models.pkl"))
    x_scalers = joblib.load(Path(full_model_path, "x_train_scaler.pkl"))

    # Check prediction mode
    if any(
        [
            prediction_mode == item
            for item in ["tcga_train_validation", "tcga_validation"]
        ]
    ):
        outer_fold_splits = joblib.load(f"{full_model_path}/cv_outer_splits.pkl")
        total_tile_selection = joblib.load(
            f"{full_model_path}/total_tile_selection.pkl"
        )[["tile_ID", "slide_submitter_id"]]
    ...
    for i in range(n_outerfolds):
        ...
        if any([prediction_mode == item for item in ["tcga_train_validation", "test"]]):
            ...
            test_slides = total_tile_selection.loc[
                test_index, "slide_submitter_id"
            ].unique()
            if slide_type == "FF":
                tiles_subset = metadata[
                    metadata["slide_submitter_id"].isin(test_slides)
                ]
