...
def get_args():
    # Script description
    description = (
        """Compute Clustering features Features: Compute proximity (between clusters)"""
    )
    ...
    parser.add_argument(
        "--slide_clusters", type=str, help="Path to csv file", required=True
    )
    parser.add_argument(
...
def compute_proximity_from_indiv_schc_between(
    ...
    # Formatting
    prox_indiv_schc = pd.merge(
        prox_indiv_schc,
        slide_indiv_clusters_labeled,
        left_on=["slide_submitter_id", "cluster1_label", "cluster1"],
        right_on=["slide_submitter_id", "cell_type_map", "cluster_label"],
    )
    ...
    prox_indiv_schc = pd.merge(
        prox_indiv_schc,
        slide_indiv_clusters_labeled,
        left_on=["slide_submitter_id", "cluster2_label", "cluster2"],
        right_on=["slide_submitter_id", "cell_type_map", "cluster_label"],
    )
    ...
    prox_indiv_schc["ordered_pair"] = [
        f"{i}-{j}"
        for i, j in prox_indiv_schc[["cluster1_label", "cluster2_label"]].to_numpy()
    ]
    prox_indiv_schc["comparison"] = [
        f"cluster1={i}-cluster2={j}"
        for i, j in prox_indiv_schc[["cluster1_is_high", "cluster2_is_high"]].to_numpy()
    ]

    # Post-processing
    results_schc_indiv = pd.concat(
        Parallel(n_jobs=n_cores)(
            delayed(features.post_processing_proximity)(
                prox_df=prox_indiv_schc,
                slide_submitter_id=id,
                method="individual_between",
            )
            for id in slide_submitter_ids
...
def main(args):
    compute_proximity_from_indiv_schc_between(
        ...
        Path(
            args.output_dir, f"{args.prefix}_features_clust_indiv_schc_prox_between.csv"
        ),
        index=False,
    )
