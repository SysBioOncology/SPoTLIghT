...
def compute_nclusters(all_slide_clusters_characterized, cell_types=DEFAULT_CELL_TYPES):
    ...
    all_slide_clusters_characterized = pd.read_csv(
        all_slide_clusters_characterized, sep=",", header=0
    )
    ...
    new_cols = [
        ("fraction {0} clusters".format(col)) for col in num_clust_slide_wide.columns
    ]
...
def main(args):
    ...
    num_clust_slide_wide.to_csv(
        Path(
            args.output_dir, f"{args.prefix}_nclusters_wide.csv", sep="\t", index=False
        )
    )
