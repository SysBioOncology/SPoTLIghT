...
def compute_proximity_from_indiv_schc_within(
    ...
    all_slide_indiv_clusters = pd.read_csv(
        slide_clusters, sep=",", header=0, index_col=False
    )
    slide_submitter_ids = list(set(all_slide_indiv_clusters.slide_submitter_id))

    slide_indiv_clusters_labeled = pd.read_csv(
        tiles_schc, sep=",", header=0, index_col=False
    )
    ...
    results_schc_indiv_within = Parallel(n_jobs=n_cores)(
    ...
    prox_indiv_schc_within = pd.merge(
        prox_indiv_schc_within,
        slide_indiv_clusters_labeled,
        left_on=["slide_submitter_id", "cell_type", "cluster1"],
    ...
    prox_indiv_schc_within = pd.merge(
        prox_indiv_schc_within,
        slide_indiv_clusters_labeled,
        left_on=["slide_submitter_id", "cell_type", "cluster2"],
        right_on=["slide_submitter_id", "cell_type_map", "cluster_label"],
    )
    prox_indiv_schc_within = prox_indiv_schc_within.rename(
    ...
    results_schc_indiv_within = pd.concat(
        Parallel(n_jobs=n_cores)(
            delayed(features.post_processing_proximity)(
                prox_df=prox_indiv_schc_within,
                slide_submitter_id=id,
                method="individual_within",
            )
            for id in slide_submitter_ids
...
def main(args):
    compute_proximity_from_indiv_schc_within(
        ...
        Path(
            args.output_dir, f"{args.prefix}_features_clust_indiv_schc_prox_within.csv"
        ),
        index=False,
    )
