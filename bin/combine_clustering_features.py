...
def get_args():
    ...
    parser.add_argument("--frac_high_wide", type=str, help="Path to csv", default="")

    parser.add_argument(
        "--num_clust_slide_wide", type=str, help="Path to csv", default=""
    )

    parser.add_argument("--all_prox_df_wide", type=str, help="Path to csv", default="")
    parser.add_argument(
...
def combine_clustering_features(
    frac_high_wide,
    ...
    prox_indiv_schc_combined_wide = pd.read_csv(
        prox_indiv_schc_combined_wide, index_col=False, header=0, sep="\t"
    )

    # Store features
    all_features = pd.merge(
        frac_high_wide, num_clust_slide_wide, on=["slide_submitter_id"]
    )
...
def main(args):
    combine_clustering_features(
        frac_high_wide=args.frac_high_wide,
        num_clust_slide_wide=args.num_clust_slide_wide,
        all_prox_df_wide=args.all_prox_df_wide,
        prox_indiv_schc_combined_wide=args.prox_indiv_schc_combined_wide,
    ).to_csv(
