...
def get_args():
    ...
    parser.add_argument(
        "--graph_features",
        type=str,
        help="Path to tab-separated file with the graph features",
        default="",
    )
...
def combine_all_spatial_features(
    graph_features,
    clustering_features,
    metadata_path="",
    is_TCGA=False,
    ...
    if is_TCGA:
        all_features_combined["TCGA_patient_ID"] = (
            all_features_combined.slide_submitter_id.str[0:12]
        )
        all_features_combined["TCGA_sample_ID"] = (
            all_features_combined.slide_submitter_id.str[0:15]
        )
        all_features_combined["sample_submitter_id"] = (
            all_features_combined.slide_submitter_id.str[0:16]
        )
...
def main(args):
    ...
    combine_all_spatial_features(
        ...
        Path(args.output_dir, f"{args.prefix}_all_features_combined.csv"),
        sep="\t",
        index=False,
    )
