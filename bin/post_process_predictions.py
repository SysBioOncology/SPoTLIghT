...
def get_args():
    ...
    parser.add_argument(
        ...
        help="Whether to create a subdirectory called 'predictions_format_parquet' if slide_type == 'FFPE', default=False",
        default=False,
    )
    parser.add_argument(
        "--slide_type",
        help="Type of tissue slide (FF or FFPE) (default='FF')",
        type=str,
        default="FF",
    )
    parser.add_argument(
...
def handle_ff_slides(pred_train_file, codebook, tissue_classes, cancer_type):
    ...
    temp = pd.DataFrame({"tumor_type": predictions["true_class_name"].str[:-2]})
...
def handle_ffpe_slides(pred_train_file, codebook, tissue_classes, cancer_type):
    ...
    predictions = predictions.map_partitions(
        lambda x: x.assign(pred_class_id=x.iloc[:, 0:41].idxmax(axis="columns"))
    )
    predictions["true_class_id"] = 41
    predictions = predictions.map_partitions(
        lambda x: x.assign(pred_probability=x.iloc[:, 0:41].max(axis="columns"))
    )
    predictions["true_class_name"] = predictions["true_class_id"].copy()
    predictions["pred_class_name"] = predictions["pred_class_id"].copy()
    ...
    predictions = predictions.map_partitions(
        lambda x: x.assign(pred_class_name=x.loc[:, "pred_class_id"].replace(res))
    )
    predictions = predictions.map_partitions(
        lambda x: x.assign(true_class_name=x.loc[:, "true_class_id"].replace(res))
    )
    predictions["is_correct_pred"] = (
        predictions["true_class_id"] == predictions["pred_class_id"]
    )
    predictions["is_correct_pred"] = predictions["is_correct_pred"].replace(False, "F")
    predictions.is_correct_pred = predictions.is_correct_pred.astype(str)
    temp = predictions.map_partitions(
        lambda x: x.assign(tumor_type=x["true_class_name"].str[:-2])
    )
    temp = temp.map_partitions(
        lambda x: pd.merge(x, tissue_classes, on="tumor_type", how="left")
    )
...
def post_process_predictions(
    pred_train_file, slide_type, path_codebook, path_tissue_classes, cancer_type
):
    """
...
def main(args):
    predictions = post_process_predictions(
        pred_train_file=args.pred_train_file,
    ...
    if args.slide_type == "FF":
        ...
        def name_function(x):
            return f"predictions-{x}.parquet"
