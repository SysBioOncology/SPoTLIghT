...
def clustering_schc_simultaneous(
    ...
    slide_clusters = Parallel(n_jobs=n_cores)(
        delayed(clustering.schc_all)(predictions, all_graphs[id], id)
        for id in slide_submitter_ids
    )
    ...
    return (
        tiles_all_schc,
        all_slide_clusters_characterized,
        formatted_tiles_all_schc,
        all_graphs,
    )
...
def main(args):
    ...
    all_slide_clusters_characterized.to_csv(
        Path(
            args.output_dir, f"{args.prefix}_all_schc_clusters_labeled.csv", index=False
        )
    )
