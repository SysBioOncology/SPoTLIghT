...
def get_args():
    ...
    parser.add_argument(
        ...
        help="Path to either a folder for multiple cancer types or single txt file.",
        required=False,
        default=None,
    )
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="generated_clinical_file.txt",
    ...
    parser.add_argument(
        "--output_dir",
        help="Path to folder for saving all created files",
        default="",
        required=False,
        type=str,
    )
    parser.add_argument(
        "--class_name",
        help="Single classname or (b) Path to file with classnames according to codebook.txt (e.g. LUAD_T)",
        default=None,
        type=str,
    )
    parser.add_argument(
...
def filter_tumor_purity(df, threshold=80):
    ...
    df["percent_tumor_cells"] = (
        df["percent_tumor_cells"].replace("'--", np.nan, regex=True).astype(float)
    )

    # Convert strings to numeric type
    df["percent_tumor_cells"] = pd.to_numeric(df["percent_tumor_cells"])
    df = df.dropna(subset=["percent_tumor_cells"])
    df = df.where(df["percent_tumor_cells"] >= float(threshold))
    return df
...
def create_TCGA_clinical_file(
    ...
    codebook_df = pd.read_csv(
        path_codebook, delim_whitespace=True, header=None, names=["class_name", "value"]
    )
    init_check_single_class = is_valid_class_name_input(
        input=class_name, codebook=codebook_df
    )
    init_check_multi_class = is_valid_class_name_input(
        input=class_names_path, codebook=codebook_df
    )
    is_single_class = init_check_single_class == "single"
    is_multi_class = init_check_multi_class == "multi"

    passes_input_check = (is_single_class | is_multi_class) & (
        clinical_files_input is not None
    )

    if passes_input_check:
        if is_multi_class:  # multi class names
            class_names = (
                pd.read_csv(class_names_path, header=None, sep="\t")
                .to_numpy()
                .flatten()
            )
            if os.path.isdir(clinical_files_input) & (len(class_names) > 1):
                clinical_file = handle_multi_class(
                    input=clinical_files_input,
            ...
            if os.path.isfile(clinical_files_input):
                clinical_file = handle_single_class(
                    input=clinical_files_input,
                    class_name=class_name,
                    codebook=codebook_df,
                )

        clinical_file = filter_tumor_purity(
            df=clinical_file, threshold=tumor_purity_threshold
        )
