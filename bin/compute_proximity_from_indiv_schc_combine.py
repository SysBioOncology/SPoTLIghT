...
def get_args():
    ...
    parser.add_argument(
        "--prox_within", type=str, help="Path to csv file", required=True
    )
    parser.add_argument(
        "--prox_between", type=str, help="Path to csv file", required=True
    )
...
def compute_proximity_from_indiv_schc_combine(prox_within, prox_between):
    ...
    prox_indiv_schc_combined.comparison = prox_indiv_schc_combined.comparison.replace(
        dict(
            zip(
                [
                    "cluster1=True-cluster2=True",
                    "cluster1=True-cluster2=False",
                    "cluster1=False-cluster2=True",
                    "cluster1=False-cluster2=False",
                ],
    ...
    prox_indiv_schc_combined["pair (comparison)"] = [
        f"{pair} ({comp})"
        for pair, comp in prox_indiv_schc_combined[["pair", "comparison"]].to_numpy()
    ]
...
def main(args):
    compute_proximity_from_indiv_schc_combine(
        prox_within=args.prox_within, prox_between=args.prox_between
    ).to_csv(
        Path(args.output_dir, f"{args.prefix}_features_clust_indiv_schc_prox.csv"),
        sep="\t",
        index=False,
    )
