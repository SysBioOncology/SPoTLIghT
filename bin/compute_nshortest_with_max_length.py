...
def compute_n_shortest_paths_with_max_length(
    tile_quantification_path,
    cell_types=None,
    graphs_path=None,
    ...
    # Post-processing
    proximity_graphs["pair"] = [
        f"{source}-{target}"
        for source, target in proximity_graphs[["source", "target"]].to_numpy()
    ]
...
def main(args):
    proximity_graphs, shortest_paths_wide, all_graphs = (
    ...
    proximity_graphs.to_csv(
        Path(args.output_dir, f"{args.prefix}_features_shortest_paths_thresholded.csv"),
        sep="\t",
        index=False,
    )
